...
class Calculator(nn.Module):
    ...
    def build(self):
        ...
        operator = []
        operator += [nn.Linear(self.num_temperature_bins, self.num_temperature_bins), nn.SiLU()]
        operator += [nn.Linear(self.num_temperature_bins, self.num_temperature_bins)]
        self.operator = nn.Sequential(*operator)

    def forward(self, x, response_function):
        x = self.model(x)
        x = x.view(-1, self.num_temperature_bins)
        x = self.operator(x)
        return 
